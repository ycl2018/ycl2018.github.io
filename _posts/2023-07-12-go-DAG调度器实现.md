...
有了图结构的支撑，我们可以开始设计调度器了。想想看，我们的调度器中需要有哪些属性？首先需要包含一个图数据结构: `dag *DAG`,其次我们需要一些并发的控制，需要引入`sync.WaitGroup`和`sync.Mutex`，然后我们需要一个状态量来标记我们的图已经构建完成了，`sealed bool`，然后我们需要感知到任务因为某些原因提前结束了，这里我们使用channel: `done chan error`加上一些其他的属性就组成的我们的调度器定义:
